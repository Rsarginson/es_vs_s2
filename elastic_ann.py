...
def run_knn_searches(num_searches=100, k=5):
    ...
    for _ in range(num_searches):
        ...
        sample_vector = [random.uniform(-1, 1) for _ in range(1536)]
        
        # Perform kNN search
        results, latency = knn_search(sample_vector, k)
        latencies.append(latency)
        
        # You can process or print individual search results here if needed
...
def print_results(results):
    ...
    for hit in results['hits']['hits']:
        ...
        print(f"Paragraph: {hit['_source']['paragraph'][:100]}...")  # Print first 100 chars of paragraph
